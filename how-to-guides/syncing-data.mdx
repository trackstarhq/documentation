...
Trackstar keeps your data fresh by periodically pulling from each connected integration, normalizing the results, and making them available through the Trackstar API and webhooks. You never have to manage the update patterns of each underlying system.
...
> **Note:** These frequencies are default settings and can be customized per connection. If you need to modify the synchronization interval for a particular data type or connection, please contact our support team for assistance.
...
To adjust the historical lookback window, navigate to your dashboard's [Connections Settings](https://dashboard.trackstarhq.com/settings/connection-settings). To pull data from before the lookback window, use an [on-demand sync](#on-demand-syncs).
...
A force sync is a manually triggered sync. It runs the same sync a schedule would — from the last sync time to now — just immediately, without waiting for the next scheduled run.
...
- **Via the Dashboard:** Go to the [Connections Page](https://dashboard.trackstarhq.com/connections), click on the connection you want to sync, and press the sync button for the desired data type.
...
- **Force syncs still work:** If you need fresh recent data for a data type while a backfill is running, you can still trigger a [force sync](#force-syncs) for it.
...
| Anything — data changed at the source | Resource events like [`order.created`](/how-to-guides/webhooks/wms/order-created) and [`order.updated`](/how-to-guides/webhooks/wms/order-updated) |
