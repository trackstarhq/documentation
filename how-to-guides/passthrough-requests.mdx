...
Passthrough is currently in beta and available for the following integrations:
...
Use passthrough requests when you need to:

- Access WMS-specific features not yet available in Trackstar's unified API
- Retrieve custom fields unique to a specific integration
- Use specialized endpoints for specific WMS platforms

For standard operations like retrieving inventory or creating orders, we recommend using Trackstar's unified API endpoints instead.

## Making a Passthrough Request

Passthrough requests are made to the `POST /wms/passthrough` endpoint. Like all Trackstar API calls, you'll need to authenticate using your API key and the access token for the specific connection.
