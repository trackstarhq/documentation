...
Passthrough is supported by default for WMS and cart integrations that connect over a standard HTTP API; integrations that sync over SFTP or another file transfer aren't supported, since there's no API to forward requests to.
