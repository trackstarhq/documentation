...
You may also set up Trackstar Link to show individual buttons for each integration. To do this, set the `integrationAllowList` prop to an array with a single integration ID, and set the `buttonId` prop to a unique value for each button.

This will bypass the integration selection screen and open the modal directly to a the specific integration's configuration step. Below is an example that loops through a list of integrations and creates a button for each one.
