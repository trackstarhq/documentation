...
Use Trackstar webhooks to be notified when a certain events occur. The timing of a webhook event related to a resource (e.g. Orders, Inventory, Products, etc.) will coincide with the [sync frequency](https://docs.trackstarhq.com/how-to-guides/syncing-data#default-sync-frequencies) of that resource.

<Note>
Webhooks are not sent when Trackstar is performing initial syncs for a connection. Webhooks are only sent for events that occur after the initial sync is complete.

For more information on how initial syncs work, see the [Syncing Data](https://docs.trackstarhq.com/how-to-guides/syncing-data#initial-sync-behavior) guide.
</Note>
...
and can be configured within the [Trackstar Dashboard](https://dashboard.trackstarhq.com/webhooks).
