...
This is deliberate. Trackstar passes your write through to the connected system, and a `null` is ambiguous: it could mean you want the field blank on purpose, or your HTTP client could be serializing every field by default with no value set. The underlying systems also disagree on what a `null` should do (some clear the field, some reject the request, some ignore it). Rather than guess and risk writing data you didn't intend, we reject the request up front. If you don't mean to set a field, leave it out of the body entirely.
