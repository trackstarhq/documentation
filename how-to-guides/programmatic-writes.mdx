...
You can also get the `meta` for an integration without an access token, using something like `GET /wms/inbound-shipments/meta/<integration_name>?method=POST`. This response may be more generic. If the response's `has_connection_specific_fields` value is `true`, you will need to provide an access token to get the full schema.
