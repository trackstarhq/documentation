...
Example UI Creation
---
The example React code below demonstrates how to use the meta endpoint to generate a form for creating an inbound-shipment in WMS A.
...
```javascript React
  ...
  const generateFormFields = (schema) => {
    return Object.keys(schema).map((key) => {
      ...
      const { description, type, enum: enumValues, items } = property;
...
const schema = {
  ...
  properties: {
    ...
    approach: {
      description:
        'How you want to process the incoming order. Either authorize the stock first, or pay the invoice first.',
      enum: ['STOCK', 'INVOICE']
    },
