...
The ```meta``` endpoint allows you to get the schema of the request body for a write request. 

All Trackstar `POST` and `PUT` endpoints have an associated ```meta``` endpoint. The ```meta``` endpoint has a required query parameter ```method```, which can be either ```POST``` or ```PUT```.
...
Write Request Flow
---
1. Call `GET wms/<object_type>/meta?method=<METHOD>` to get the schema of the request body.
