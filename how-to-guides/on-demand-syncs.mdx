...
| `stuck` | The job stopped making progress for an extended period. A [`connection.sync-job-stuck`](/how-to-guides/webhooks/admin/connection-sync-job-stuck) webhook is emitted when this happens. Stuck jobs do not resume on their own — [reach out to us](mailto:support@trackstarhq.com) and we'll investigate. |

A typical successful lifecycle is `open → completed`. A cancelled job follows `open → cancelled`, and a job that stops progressing moves `open → stuck`. Only jobs in the `open` state can be cancelled.
