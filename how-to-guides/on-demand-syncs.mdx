...
- **Asynchronous execution:** Sync jobs run in the background. Poll the status endpoint or rely on Trackstar's existing webhooks (e.g. `wms.order-created`, `wms.order-updated`) to know when new data has landed.
