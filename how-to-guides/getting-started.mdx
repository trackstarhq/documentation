...
description: "Get an API key, connect a customer system with Trackstar Link, and make your first API call"
---
...
<Tip>
  Building with Claude Code, Cursor, or another coding agent? [Build with AI Agents](/how-to-guides/build-with-ai) has ready-made prompts for the steps below, plus these docs in formats an agent can read.
</Tip>
...
<Note>
  `available_actions` lists the endpoints available on the connection. Endpoints your org has [disabled](/how-to-guides/syncing-data#disabled-endpoints) are omitted.
</Note>
