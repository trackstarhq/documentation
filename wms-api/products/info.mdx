...
Each `inventory_item` returned from the Product endpoint has an `inventory_item_id` that maps to the `id` field in the [Inventory](/wms-api/inventory/info) endpoint.
...
Then, call the [`/wms-api/inventory/{inventory_item_id}`](/wms-api/inventory/get-item) endpoint to get the inventory levels for each `inventory_item_id`.
